...
const SignUpPage = () => {
  ...
  const triggerGoogleOAuth = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      setLoading(true);
      const toastId = toast.loading("Connecting to Google Account...");
      try {
  ...
  const handleMockGoogleLogin = async (mockData) => {
    ...
    const toastId = toast.loading("Registering via Google Account...");
    try {
      await googleLogin({ isMock: true, mockPayload: mockData });
      toast.success("Successfully registered via Google!", { id: toastId });
      navigate("/dashboard");
    } catch (err) {
  ...
  const handleSendOTP = async () => {
    ...
    try {
      ...
      if (res.otp) {
        ...
        // Autofill for smooth testing UX or manual entry
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    const toastId = toast.loading("Creating your account & signing in...");
    ...
    try {
      if (authMethod === "password_email") {
        ...
        const identifier = isEmail ? formData.email.trim() : formData.mobileNumber.trim();
        const type = isEmail ? "email" : "mobile";
        await verifyOTP(identifier, formData.otpCode.trim(), type, formData.fullName.trim());
      }

      toast.success("Account created successfully!", { id: toastId });
      navigate("/dashboard");
    } catch (err) {
      toast.error(err.message || "Registration failed. Please check the code.", {
        id: toastId,
      });
    } finally {
      setLoading(false);
        ...
        <div className="w-full flex flex-col items-center">
          ...
          <div className="w-full flex flex-col items-center text-center mb-8">
            <div className="relative inline-block">
              {/* Soft purple dot overlapping Sign Up title */}
          ...
          <form onSubmit={handleSubmit} className="w-full flex flex-col gap-4">
            ...
            <div className="w-full flex flex-col items-start gap-1">
              ...
              <input
                ...
                className={`w-full h-[46px] px-4 bg-bg-secondary text-text-primary border ${
                  errors.fullName ? "border-red-400 focus:border-red-500 focus:ring-red-200/30" : "border-border-primary focus:border-brand-purple focus:ring-brand-purple/20"
            ...
            {/* Email Address (Visible for otp_email and password_email) */}
            ...
            {/* Mobile Number (Visible for otp_mobile) */}
            {authMethod === "otp_mobile" && (
              <div className="w-full flex flex-col items-start gap-1">
                <label className="text-[12px] font-bold text-text-primary">
                  Mobile Number
            ...
            {/* Password Fields (Visible for password_email) */}
            {authMethod === "password_email" && (
              <>
                <div className="w-full flex flex-col items-start gap-1">
                  <label className="text-[12px] font-bold text-text-primary">
                ...
                <div className="w-full flex flex-col items-start gap-1">
                  ...
                  <div className="w-full relative">
                    <input
                      ...
                      disabled={loading}
                      className={`w-full h-[46px] pl-4 pr-11 bg-bg-secondary text-text-primary border ${
                        errors.confirmPassword ? "border-red-400 focus:border-red-500 focus:ring-red-200/30" : "border-border-primary focus:border-brand-purple focus:ring-brand-purple/20"
                      } rounded-[10px] text-xs font-sans placeholder-text-muted focus:ring-1 outline-none transition-all duration-300 disabled:bg-bg-tertiary disabled:text-text-muted`}
                      required
                    />
                    <button
      ...
      >
        <div className="flex flex-col gap-5 text-left">
          <p className="text-[11px] text-text-secondary font-medium leading-relaxed">
            Since no Google Client ID is configured in <code className="text-text-primary bg-bg-secondary border border-border-primary px-1 rounded font-mono">Frontend/eBookCreator/.env</code>, you can test the database onboarding, credentials registration, and OAuth profile syncing via this interactive emulator:
          </p>
          ...
          <div className="flex flex-col gap-2.5">
            ...
            {[
              ...
              <button
                ...
                type="button"
                className="w-full flex items-center justify-between p-3.5 border border-border-primary hover:border-text-primary hover:bg-bg-secondary rounded-xl transition-all duration-200 text-left group cursor-pointer bg-bg-primary"
              >
                <div className="flex items-center gap-3">
                  <img
                    src={account.picture}
                    alt={account.name}
                    className="w-9 h-9 rounded-full border border-border-primary shadow-sm"
                  />
                  <div>
                ...
                <span className="text-[9px] font-extrabold text-emerald-600 uppercase tracking-wider bg-emerald-50/10 px-2 py-0.5 rounded-full">
                  Sign In
                </span>
              </button>
            ))}
          ...
          <div className="flex items-center gap-3 my-1">
            <div className="flex-1 h-[1px] bg-border-primary"></div>
            <span className="text-[8px] font-extrabold uppercase tracking-widest text-text-muted">
              OR CUSTOM ACCOUNT
            </span>
            <div className="flex-1 h-[1px] bg-border-primary"></div>
          </div>
          ...
          <div className="flex flex-col gap-3">
            ...
            <Button
              ...
              type="button"
              variant="primary"
              className="w-full h-[45px] bg-brand-purple hover:bg-purple-700 text-white text-[10px] font-bold tracking-wider rounded-xl transition-all duration-300 border-none cursor-pointer"
            >
